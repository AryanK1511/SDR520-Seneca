...
// At this distance, we know that we need to start turning the bot in some direction
float K_sonar_threshold_distance_cm = 50;

// ========== CONSTANTS ==========

// Ref Doc for robot specifications: https://www.robotc.net/teaching_rc_vex_iq/lesson/media_files/PalmIslandGuide.pdf
...
// This variable increments at every turn so that we know whether to turn left or right
// For this current program, we turn left when this var is 0 and 1 and right when this is 2
int turn_count = 0;
// Turn 90 degrees
float turn_angle = 90;
...
int required_turn_ticks = arc_length * (C_encoder_pulses_per_rev / C_wheel_circumference_cm);
...
{ 
	while (!(is_collision())) {
		...
		// Increment the turn counter every time the robot turns to know which direction to turn in
		turn_count += 1;
	};
